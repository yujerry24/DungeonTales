...
public class DungeonTales extends JFrame {
    ...
    public DungeonTales() {
        ...
        Timer movement = new Timer(5, new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                ...
                for (int i = 0; i < pressed.length; i++) {
                    ...
                    if (pressed[i] == KeyEvent.VK_LEFT) {
                        p.setX(p.getX() - 3);
                    } else if (pressed[i] == KeyEvent.VK_RIGHT) {
                        p.setX(p.getX() + 3);
                    }
                }
            }
        ...
        Timer gravity = new Timer(2, new ActionListener() {
            ...
            public void actionPerformed(ActionEvent arg0) {
                if (doGravity) {
                    ...
                    for (Rectangle r : p.getCurrentLevel().getPlatforms()) {
                        if (p.getY() == r.getY() - 120) {
                            if (p.getX() + 170 / 2 >= r.x && p.getX() <= r.x + r.getWidth() - 110 / 2) {
                                return;
                            }
                        }
                    }

                    for (Platform pl : p.getCurrentLevel().getMovingPlats()) {
                        if (pl == null) {
                            return;
                        }

                        Rectangle r = new Rectangle(pl.getX(), pl.getY(), pl.width, pl.height);

                        if (p.getY() >= r.getY() - 120 && p.getY() <= r.getY() - 20) {
                            if (p.getX() + 170 / 2 >= r.x && p.getX() <= r.x + r.getWidth() - 110 / 2) {
                                p.onPlat = true;
    ...
    static Timer plat = new Timer(20, null);
